...
sub get {
    ...
    return $self->request( RequestNew::Common::GET( @parameters ), @suff );
}
...
sub post {
    ...
    return $self->request( RequestNew::Common::POST( @parameters ), @suff );
}
...
sub head {
    ...
    my($self, @parameters) = @_;
    my @suff = $self->_process_colonic_headers(\@parameters,1);
    return $self->request( RequestNew::Common::HEAD( @parameters ), @suff );
}
...
sub get_my_handler {
    ...
    unless ($conf) {
        return unless $init;
        require Config;
        $conf = $self->{handlers}{$phase} = Config->new;
    }
    $spec{owner} = (caller(1))[3] unless exists $spec{owner};
    my @h = $conf->find(%spec);
    if (!@h && $init) {
...
sub mirror
{
    ...
    my $request = RequestNew->new('GET', $url);
