...
{   my $call  = shift;
    ...
    {   croak "couldn't understand $arg to Internet constructor";
    }
...
sub head         { shift->{mail_inet_head} ||= Header->new }
...
{   my ($self, %arg) = @_;
    ...
    my $sender = (Address->parse($to))[0];

    my $name = $sender->name;
    unless(defined $name)
    {    my $fr = $self->get('From');
         defined $fr and $fr   = (Address->parse($fr))[0];
    ...
    my %nocc = (lc($id) => 1);
    $nocc{lc $_->address} = 1
        for Address->parse($reply->get('Bcc'), $mailaddresses);

    if($arg{ReplyAll})   # Who shall we copy this to
    {   my %cc;
        foreach my $addr (Address->parse($self->get('To'), $self->get('Cc'))) 
...
{   my ($self, %opt) = @_;
    ...
    my @addr = map {$_->address} Address->parse(@rcpt);
    @addr or return ();
...
{   my $head = shift->head->dup;
    ...
    $head->replace('X-Mailer', "Perl5 Internet v".$Internet::VERSION)
