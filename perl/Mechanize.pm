...
use RequestNew 1.30;
use UserAgentNew 5.827;
use FormNew 1.00;
use HTML::TokeParser;

use base 'UserAgentNew';
...
sub new {
    ...
    my %mech_parms = (
        autocheck   => ($class eq 'Mechanize' ? 1 : 0),
        onwarn      => \&Mechanize::_warn,
        onerror     => \&Mechanize::_die,
        quiet       => 0,
...
sub get {
    ...
    $uri = $uri->url if ref($uri) eq 'Mechanize::Link';
...
sub _SUPER_put {
    require RequestNew::Common;
    my($self, @parameters) = @_;
    my @suff = $self->_process_colonic_headers(\@parameters,1);
    return $self->request( RequestNew::Common::PUT( @parameters ), @suff );
}
...
sub select {
    ...
    if (ref($value) eq 'HASH') {
        ...
        if (defined($value->{n})) {
            ...
            # (see INPUTS section of `perldoc FormNew`)
            if (@inputs == 1) {
...
sub update_html {
    ...
    $self->{forms} = [ FormNew->parse($html, $self->base) ];
    for my $form (@{ $self->{forms} }) {
